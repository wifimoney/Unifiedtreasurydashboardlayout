...
contract BudgetAllocator {
    ...
    // Employee to department mapping (for payroll integration)
    mapping(address => uint256) public employeeToDepartment;
    mapping(address => bool) public isEmployeeAssigned;
    ...
    event EmployeeAssigned(address indexed employee, uint256 indexed departmentId);
     ...
     * @param _payrollManager PayrollManager contract address
     */
    function setPayrollManager(address _payrollManager) external onlyAdmin {
        require(_payrollManager != address(0), "Invalid payroll manager");
    ...
    /**
     * @notice Assign employee to department (called by PayrollManager or Admin)
     * @param _employee Employee address
     * @param _departmentId Department ID
     */
    function assignEmployeeToDepartment(address _employee, uint256 _departmentId)
        external
        departmentExists(_departmentId)
    {
        require(msg.sender == admin || msg.sender == payrollManager, "Not authorized");
        require(_employee != address(0), "Invalid employee");
        require(departments[_departmentId].active, "Department not active");

        employeeToDepartment[_employee] = _departmentId;
        isEmployeeAssigned[_employee] = true;

        emit EmployeeAssigned(_employee, _departmentId);
    }

    /**
     * @notice Check if department has sufficient budget
     * @param _departmentId Department ID
     * @param _amount Amount to check
     * @return bool True if budget available
     */
    function checkBudgetAvailable(uint256 _departmentId, uint256 _amount)
        external
        view
        departmentExists(_departmentId)
        returns (bool)
    {
        Department storage dept = departments[_departmentId];
        return dept.active && dept.availableBalance >= _amount;
    }

    /**
     * @notice Get department ID for an employee
     * @param _employee Employee address
     * @return uint256 Department ID
     */
    function getDepartmentForEmployee(address _employee) external view returns (uint256) {
        require(isEmployeeAssigned[_employee], "Employee not assigned to department");
        return employeeToDepartment[_employee];
    }
